...
两种写法的三点不同：r的初始值，while循环条件，r的移动规则（l都是mid+1）[参考](https://leetcode-cn.com/problems/binary-search/solution/er-fen-cha-zhao-de-xun-huan-bu-bian-liang-zhi-yao-/)
...
[四道旋转数组](https://www.cnblogs.com/lfri/p/12553209.html)
...
[方法三-非递归方法](https://leetcode-cn.com/problems/median-of-two-sorted-arrays/solution/xun-zhao-liang-ge-you-xu-shu-zu-de-zhong-wei-s-114/)

【LeetCode 352】

<img src="../../../../学习/笔记合集/img/image-20211010193013470.png" alt="image-20211010193013470" style="zoom:25%;" />
...
遇到有递推特征的题目 - 想递归和DP（递归一般接近于暴搜，但通过优化一般能ac，如果递归不行就要用DP）

DP比较重要的是找规律，可以从简单的例子开始分析找规律总结状态转移方程
...
public:
    /**
    * 两个注意点：遍历的时候不能用普通遍历，而是要通过长度来进行遍历，因为如果普通遍历的话dp[i-1][j+1]还没有计算
    * 第19行，还需要判断i，j是否相邻，如果相邻且相等肯定为回文串
    */
    string longestPalindrome(string s) {
        ...
        for (int L = 2; L <= n; L++) { //长度
            for (int i = 0, j = 0; i < n - L + 1; i++) {
                ...
                if (dp[i][j]) {
                    int len = j - i + 1;
                    if (len > res.size()) res = s.substr(i, len);
                }
            }
        }
        return res;
...
public:
    int minimumTime(int n, vector<vector<int>>& relations, vector<int>& time) {
        ...
        vector<int> dp(n + 1); //到达节点i的最长所需时间
        for (auto relation : relations) {
            degrees[relation[1]]++;
            edges[relation[0]].push_back(relation[1]);
        }

        for (int i = 1; i <= n; i++) { //课程编号从1到nn
            if (degrees[i] == 0) {
                dp[i] = time[i-1];
                q.push(i);
                res = max(res, dp[i]);
...
递归函数的path形参，如果函数内有修改，最好不要加引用，如【LeetCode22】，否则后面的运算是在前面计算好的path进行的

相关题目：

【LeetCode17】直接按模版写出来即可
...
public:
    ...
    vector<string> letterCombinations(string digits) {
        ...
        set<string> st;
        string path = "";
        helper(digits, st, path, 0);
        vector<string> res;
        res.assign(st.begin(), st.end());
        return vector<string>(res);
    }
    void helper(string digits, set<string>& st, string& path, int pos) {
        if (digits.size() == pos) {
            st.insert(path);
            return;
        }
        for (char c : phone[digits[pos] - '0' - 2]) {
            path.push_back(c);
            helper(digits, st, path, pos + 1);
            path.pop_back();
        }
    }
};
...
public:
    vector<string> generateParenthesis(int n) {
        vector<string> res;
        string path = "";
        helper(0, 0, path, res, n);
        return res;
    }
    void helper(int l, int r, string& path, vector<string>& res, int n) {
        string brace = "()";
        if (path.size() == n * 2) {
            res.push_back(path);
            return;
        }
      // //原来我自己分的情况是r == l和 r < l，然后在r < l情况下可以加左或者右，但其实回溯的过程也指定了顺序，先l<n然后r<l也可以
        if (l < n) { 
            path.push_back('(');
            helper(l + 1, r, path, res, n);
            path.pop_back();
        }
        if (r < l) {
            path.push_back(')');
            helper(l, r + 1, path, res, n);
            path.pop_back();
        }
    }
};
...
public:
    vector<vector<int>> combinationSum(vector<int>& candidates, int target) {
        ...
        helper(candidates, res, path, 0, target, 0);
        return res;
    }
    void helper(vector<int> candidates, vector<vector<int>> &res, vector<int>& path, int curSum, int target, int ind) {
        ...
        for (int i = ind; i < candidates.size(); i++) {
            if (curSum + candidates[i] <= target) {
                ...
                helper(candidates, res, path, curSum + candidates[i], target, i);//注意这里是i，第一次做题时误写为ind
                path.pop_back();
            } else break;
        }
    }
  ...
  void helper(vector<int> candidates, vector<vector<int>> &res, vector<int>& path, int curSum, int target, int ind) {
        ...
        helper(candidates, res, path, curSum, target, ind + 1);
        if (curSum + candidates[ind] <= target) {
            path.push_back(candidates[ind]);
            helper(candidates, res, path, curSum + candidates[ind], target, ind);
            path.pop_back();
        }
    }
};
...
数组被分成两个部分，左边表示已经填过的数，右边表示待填的数，我们在回溯的时候只要动态维护这个数组即可

用sep来表示当前已经排列了几位，并通过swap操作，把已经用过的数字移动到左边。也就是对于数组`nums`，[0, sep)是已经用过，待选数字是[first, n)。

比如数组[1,2,3]，当sep等于1时候，表示最左边1位已经用过，那么后续的组合就从sep开始
...
public:
    ...
    void helper(vector<vector<int>>& res, vector<int>& path, vector<int> nums, int sep) {
        if (sep == nums.size()) {
            res.push_back(path);
            return;
        }
        //sep是分隔符，sep左边是用过的数字，表示填到第几个数
        for (int i = sep; i < nums.size(); i++) {
            swap(path[i], path[sep]);
            helper(res, path, nums, sep + 1);
            swap(path[i], path[sep]);
        }
    }
};
...
<img src="../../../../学习/笔记合集/img/image-20220128100637621.png" alt="image-20220128100637621" style="zoom: 33%;" />

如果题目要求字典序排列结果，那么可以通过一个标记数组vis，来记录哪些已经被排列过
...
public:
    int shoppingOffers(vector<int>& price, vector<vector<int>>& special, vector<int>& needs) {
        ...
        for (auto coupon : special) {
            bool isVaild = true;
            for (int i = 0; i < n; i++) {
                if (needs[i] < coupon[i]) isVaild = false; //不能使用该优惠券
                needs[i] -= coupon[i];
            }
...
增加当前数字的语句，即`sum += nums[right]`，也可能放在内循环之后，比如【LeetCode3】，先把set中与右边界相同的数字去掉，然后就可以把右边界放进窗口了。
...
有序数组需要想到用双指针，通过两指针配合实现去重、得到目标和的目的
        ...
        for (int i = 0; i < n - 2; i++) {
            ...
            while (l < r) {
                if (nums[l] + nums[r] == target) {
                    ...
                    while (nums[l] == b && l < r) l++;
                    while (nums[r] == c && l < r) r--;
...
本题的重点是保证不重复，因此第二三层循环需要进行去重处理，因此先排序，再用双指针
...
1. 下一个比当前大的数：需要把后面的大数和前面的交换
2. 增加的幅度尽可能小：
   1. 交换的数字尽可能靠右 -> 从后向前查找
   2. 尽可能小的「大数」和前面的「小数」交换
   3. 交换后需要重置为升序

具体实现：

1. 从后向前找到一个相邻的升序对i j，满足nums[i] < nums[j]（我的理解是，有升序才能有交换）
2. 从后向前在[j, end)找出第一个满足nums[i] < nums[k]的元素，i对应小数，k对应大数
3. 交换i，j
4. 此时[j, end]肯定逆序，需要对其反转
...
public:
    void nextPermutation(vector<int>& nums) {
        ...
        if (n == 1) return;
        int i = n - 2, j = n - 1, k = 0;
        while (i >= 0 && nums[i] >= nums[j]) { //注意这里的等号以及i>=0的判断在左边形成短路
            i--; j--;
        }

        if (i >= 0) {
            k = n - 1;
            while (nums[k] <= nums[i] && k >= j) { //注意这里的等号
                k--;
            }
            swap(nums[i], nums[k]);
        }
...
public:
    int trap(vector<int>& height) {
      ...
      while (l < r) {
        ...
        if (height[l] < height[r]) { //这里改成lMax < rMax也能AC
          res += lMax - height[l++];
        } else {
          res += rMax - height[r--];
        }
...
public:
    int trap(vector<int>& height) {
      ...
      for (int i = 0; i < n; i++) {
        while (!st.empty() && height[i] > height[st.top()]) { //i是右高
          int low = st.top(); st.pop(); //中间低 
          if (st.empty()) break;
          int left = st.top(); //左高
          int curHeight = min(height[left], height[i]) - height[low];
          res += (i - left - 1) * curHeight;
        }
        st.push(i); //如果当前高度小于栈顶高度，那么说明有积水
      }
    }
};
```
...
这类题需要注意最高位的进位，可能需要最后结果多加一位，比如9999+99999，最后结果是六位数
...
可以转化为位运算，一位一位地进行计算，每位可以根据两个位分为三种情况，同为1，只有一个为1，同为0，根据三种情况更新carry和res。
...
public:
    int getSum(int a, int b) {
        int res = 0, carry = 0;
        for (int i = 0; i < 32; i++) {
            // int ca = a & (1 << i), cb = b & (1 << i);   是用来判断当前位是否为1和0的方法，结果可能大于1
            int ca = (a >> i) & 1, cb = (b >> i) & 1; //把a右移，再和1与运算，最后结果只能是0/1
            if (ca == 1 && cb == 1) {
                res |= (carry << i);
                carry = 1;
            } else if (ca == 1 || cb == 1){
...
0 1 0 1 ^ 0 1 0 0 = 0 0 0 1（相当于二者相加，并且没有进位）

与操作的特性：可以得到进位

0 1 0 1 & 0 1 0 0 = 0 1 0 0 然后再左移一位

因此不断循环这个过程，直到进位是0（因为做完一次后，还可能有新进位）

注意要用unsigned int的原因是LeetCode的编译器不能对负数左移，也就是最高位必须是0才能左移。所以用unsigned int 或者与一个最高位为0，其他位都是1的数，这样可以强制将最高位清零，然后再进行左移。
...
public:
    int getSum(int a, int b) {
      ...
      while (b != 0) {
        carry = (unsigned int)(a & b) << 1; //或者carry = (a & b & 0x7fffffff) << 1;
      	a = a ^ b;
        b = carry;
      }
      return a;
...
总结：子序列问题一般都需要二重循环，第一重循环从0到n遍历状态，第二重循环向前找**上一个子序列的结尾**j(0 <= j < i) 也就是把满足条件的i，加入到j的子序列中(上一个状态)
...
public:
    ...
    int findNumberOfLIS(vector<int>& nums) {
      int dp[N], count[N], len = 0, res = 0; //dp[i]表示以nums[i]结尾的LIS长度，count[i]表示以nums[i]结尾的LIS个数
      int n = nums.size();
      for (int i = 0; i < n; i++) {
        dp[i] = count[i] = 1; //注意需要初始化
        for (int j = 0; j < i; j++) {
          if (nums[j] < nums[i]) {
            if (dp[i] < dp[j] + 1) {
              count[j] = count[i];
            ...
            else if (dp[i] == dp[j] + 1) count[j] += count[i];
          }
        }
        len = max(len, dp[i]);
      }
...
相关题目： https://leetcode-cn.com/problems/number-of-longest-increasing-subsequence/solution/gong-shui-san-xie-lis-de-fang-an-shu-wen-obuz/
...
重要性质：**当数组中每个元素各不相同，LCS问题可以转化为LIS问题**

> 由于各不相同，因此元素和下标一一对应，可以存在哈希表中

【LeetCode 1713】【好题】

直观来看：最小操作次数 = target长度 - 最长子序列长度
...
public:
    int minOperations(vector<int>& target, vector<int>& arr) {
        ...
        vector<int> dp;
      //接下来对arr求LIS，但没有必要对arr实际进行转换，只需要遍历的时候，如果某个元素在target出现过，判断是否加入LIS即可
        for (int a : arr) {
            if (m.count(a)) {
                int val = m[a];
...
dp[i]表示以`nums[i]`结尾最长递增子序列长度，递推式是`dp[i] = max(dp[i], dp[j]+1)`

```c++
int lengthOfLIS(vector<int>& nums) {
  int res = 0, n = nums.size();
  vector<int> dp(n, 1); //注意需要初始化为1
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < i; j++) {
      if (nums[i] > nums[j]) {
...
每次遍历过程中：找目标为大于等于`nums[i]`的元素，如果不存在，那么可以把`nums[i]`放在序列的最后面，相当于序列长度++；如果存在，那么需要对这个元素完成替换，因为需要尽可能替换为小的元素
...
用边来表示有无字符，用点来记录是否为单词结尾和后续字符串的字符
...
把运算转为long long的原因是题目说负数最小可以取到$$2^{31}$$，因此需要扩展到long long

计算小数位的时候的技巧：把余数乘10，再除以除数，得到的商就是下一位数字（其实就和手动除法一样）
...
public:
    string fractionToDecimal(int numerator, int denominator) {
        ...
        quo = num / den;
        rem = num % den;
        unordered_map<ll, int> m; //存储小数部分每个数字出现的位置
        res += to_string(quo);
        if (rem == 0) return res;
        res += ".";
        string s = "";
        int pos = 0;
        while (rem != 0) {
            if (m.find(rem) != m.end()) { //如果当前数字出现过，说明小数部分已经完成一次循环
                s.insert(m[rem], "(");
                s += ")";
                return res + s;
            }
...
- 能以O(1)找到需要的元素，减少一次for循环，主要解决存在性问题
...
哈希表不是创建好后直接为元素计数，而是用了一个比较巧妙的方法，当哈希表中有符合条件的元素，直接返回；如果没有，就把当前元素存储，然后继续往后遍历
